...
def get_font(family: str = "arial", size: int = 14):
    # Check if font exists and return it, if not return default system font
    if pygame.font.get_fonts().__contains__(family):
        return pygame.font.SysFont(family, size)

    return pygame.font.get_default_font()


# PyGame Button implementation, source taken from:
...
class PygButton(object):
    def __init__(self, rect=None, caption='', bgcolor=LIGHTGRAY, fgcolor=BLACK, font=None,
        ...
        self.customSurfaces = False  # button starts as a text button instead of having custom assets for each surface
        ...
        if normal is None:
            ...
            self._update()  # draw the initial button assets
        else:
